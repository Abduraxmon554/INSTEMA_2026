...
- **API (Express + PostgreSQL):** http://localhost:3001
...
## Backend, baza va xavfsizlik

Backend — `server.js` (Express). `DATABASE_URL` berilsa PostgreSQL (Render),
aks holda lokal `data/db.json` ishlatiladi.

- **Login** serverda tekshiriladi (`POST /api/login`), parollar `scrypt` bilan hash'lanadi.
  Eski ochiq matndagi parollar server ishga tushganda avtomatik hash'lanadi.
- **Admin token** talab qiladigan yo'llar: `GET/PATCH/DELETE /registrations`, `GET /certificates`.
- Ochiq yo'llar: `POST /registrations`, `GET/POST /reviews`, `GET /schedule`, `GET /website`
  (`POST` yo'llar cheklangan va tekshiriladi).
- **Muhit o'zgaruvchilari**: `DATABASE_URL`, `AUTH_SECRET`, `ADMIN_USERNAME`,
  `ADMIN_PASSWORD`, `CORS_ORIGIN`, `VITE_API_URL` — `.env.example` ga qarang.
